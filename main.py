...
class ProductDetectionSystem:
    def __init__(self, config):
        ...
        self.product_matcher = UnifiedProductMatcher(config['db_path'], use_vector_db=config['use_vector_db'])

        if config['use_bert']:
            self.bert_tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
            self.bert_model = BertForMaskedLM.from_pretrained('bert-base-uncased')

        self.known_products = set(p['brand'] for p in self.product_matcher.products) if self.product_matcher.products else set()
        if not self.known_products:
            logging.warning("No products loaded. Detection may not work correctly.")
    ...
    def process_frame(self, frame):
        ...
        try:
            ...
            if self.ocr_processor:
                ocr_frame = self.preprocess_for_ocr(frame)
                ocr_results = self.ocr_processor.perform_ocr(ocr_frame)
                for bbox, text, conf in ocr_results:
                    if conf > self.ocr_threshold:
                        if self.config['use_bert']:
                            text = self.apply_bert_correction(text)
                        matched_product, confidence = self.product_matcher.match_product(text)
                        if matched_product:
                            product_name = f"{matched_product['brand']} {matched_product['flavor']} {matched_product['size']}"
                            all_detections.append({
                                'bbox': bbox,
                                'class': product_name,
                                'conf': confidence,
            ...
            frame_with_boxes = self.draw_detections(frame, all_detections)
            return frame_with_boxes, all_detections

        except Exception as e:
            logging.error(f"Frame processing failed: {str(e)}", exc_info=True)
            return frame, []
    ...
    def apply_bert_correction(self, text):
        inputs = self.bert_tokenizer(text, return_tensors="pt")
        with torch.no_grad():
            outputs = self.bert_model(**inputs)
        predicted_token = outputs.logits.argmax(dim=-1)
        corrected_text = self.bert_tokenizer.decode(predicted_token[0])
...
def process_video(system, video_path, max_frames=None):
...
def evaluate_detections(all_detections, ground_truth):
    detection_counts = Counter(d['class'] for d in all_detections)

    true_positives = sum(detection_counts[product] for product in ground_truth if product in detection_counts)
    false_positives = sum(detection_counts.values()) - true_positives
    false_negatives = len(ground_truth) - len(set(ground_truth) & set(detection_counts.keys()))

    precision = true_positives / (true_positives + false_positives) if (true_positives + false_positives) > 0 else 0
    recall = true_positives / (true_positives + false_negatives) if (true_positives + false_negatives) > 0 else 0
    f1_score = 2 * (precision * recall) / (precision + recall) if (precision + recall) > 0 else 0

    yolo_detections = [d for d in all_detections if d['detection_type'] == 'YOLO']
    ocr_detections = [d for d in all_detections if d['detection_type'] == 'OCR']

    return {
        'total_detections': len(all_detections),
        'unique_products_detected': len(detection_counts),
...
def main():
    ...
    for config in configurations:
        ...
        experiment_results = run_experiment(config, video_path, ground_truth, max_frames)
    ...
    logging.info(f"Experiment results saved to experiment_results_{timestamp}.json")

    print("\nExperiment Results Summary:")
    for config_name, result in results.items():
        print(f"\n{config_name}:")
        print(f"  Total Detections: {result['total_detections']}")
        print(f"  Unique Products Detected: {result['unique_products_detected']}")
        print(f"  YOLO Detections: {result['yolo_detections']}")
        print(f"  OCR Detections: {result['ocr_detections']}")
        print(f"  Precision: {result['precision']:.2f}")
        print(f"  Recall: {result['recall']:.2f}")
        print(f"  F1 Score: {result['f1_score']:.2f}")
        print(f"  Average Confidence: {result['average_confidence']:.2f}")
        print("  Detection Breakdown:")
        for product, count in result['detection_breakdown'].items():
