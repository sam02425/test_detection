...
class ProductDetectionSystem:
    ...
    def process_frame(self, frame):
        ...
        for detection in yolo_detections:
            matched_product, confidence = self.product_matcher.match_product(detection['class'])
            if matched_product:
...
def process_webcam(system):
    logging.info("Processing webcam feed")
    ...
    while True:
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'):
            logging.info("Webcam processing stopped by user")
            break

    cap.release()
    cv2.destroyAllWindows()
    logging.info(f"Webcam processing complete. Total frames processed: {frame_count}")
    logging.info(f"Total detections accumulated: {len(all_detections)}")
...
def main():
    ...
    try:
        ...
        if use_webcam:
            ...
            video_path = 'data/test5.mp4'  # Replace with your video path
        ...
        final_product, final_confidence = get_final_product(all_detections, threshold=0.5)
        ...
        if 'system' in locals() and hasattr(system.product_matcher, 'close'):
            system.product_matcher.close()
